...
async function getOrderInfo(orderId: string) {
  ...
  orderInfo.productInfo = await axios
    .get(productEndpointUrl, {
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
        'X-Auth-Token': process.env.BIGCOMMERCE_STORE_API_TOKEN || '',
...
export default async function handler(
  ...
  try {
    ...
    const cartData = await getCart(cartId).catch(() =>
      console.log('No cart data to get')
    )
    ...
    console.log(
      'start eventData:__________________________',
    ...
    if (scope === 'store/cart/converted') {
      ...
      const { billing_address, ip_address } = orderInfo
    }
