...
class Attributor {
  ...
  constructor(config?: AttributorConfig) {
    ...
    if (!this.cookie.get(this.config.cookieNames.first)) {
      // @ts-ignore
      this.cookie.set(
        this.config.cookieNames.first,
        JSON.stringify(this.attributionData.first),
    ...
    if (!this.cookie.get(this.config.cookieNames.last)) {
      // @ts-ignore
      this.cookie.set(
        this.config.cookieNames.last,
        JSON.stringify(this.attributionData.last),
        365
      )
    }
  }
  ...
  private readFirstAttribution(): Attribution {
    ...
    for (const field in fieldMap) {
      ...
      attribution[field as keyof Attribution] = this.getParam(
        queryString,
        queryParam
      )
    }
  ...
  public initialize(): void {
    ...
    if (!firstVisitTime) {
      ...
      this.session.last = false
      // @ts-ignore
      this.cookie.set(firstVisitCookieName, Date.now().toString(), {
        expires: 30,
        domain: cookieDomain,
      })
      // @ts-ignore
      this.cookie.set(lastVisitCookieName, Date.now().toString(), {
        expires: 365,
        domain: cookieDomain,
      })
      return
