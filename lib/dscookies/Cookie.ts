...
class Cookie {
      ...
      if (typeof ctxOrCookie === 'string') {
        ...
        this.props = {
          cookie: new UniversalCookie(
            (ctxOrCookie as NextApiRequestResponse).req.headers.cookie
          ),
          isServer,
          ctx: ctxOrCookie,
        }
  ...
  set(name: string, value: any, options?: CookieSetOptions): void {
    ...
    if (this.props.isServer && this.props.ctx) {
      const cookies: string[] =
        (this.props.ctx.res.getHeader(SET_COOKIE_HEADER) as string[]) || []

      this.props.cookie.set(name, value, options as CookieSetOptions)
      cookies.push(
        parser.serialize(name, value, options as parser.SerializeOptions)
      )
  ...
  remove(name: string, options?: CookieSetOptions): void {
    ...
    if (this.props.isServer && this.props.ctx) {
      this.props.ctx.res.setHeader(
        SET_COOKIE_HEADER,
        parser.serialize(name, '', opt as parser.SerializeOptions)
      )
    } else {
